export class InventoryPanel {
  constructor({
    ...
    this.quantityElement = rootElement.querySelector(
      "[data-inventory-quantity]",
    );
    this.descriptionElement = rootElement.querySelector(
    ...
    this.renderItemVisual = renderItemVisual;
    this.visualCanvases = [];
  ...
  createVisualCanvas(itemId, size, className) {
    const canvas = document.createElement("canvas");
    canvas.width = size;
    canvas.height = size;
    canvas.className = className;
    canvas.dataset.inventoryVisual = itemId;
    this.visualCanvases.push(canvas);
    return canvas;
  }
  ...
  render(inventoryState, itemDefinitions, selectedItemId) {
    ...
    for (const itemId of itemIds) {
      ...
      const icon = this.createVisualCanvas(itemId, 44, "inventory-item-icon");
    ...
    if (selectedItemId === null) {
      ...
      delete this.iconElement.dataset.inventoryVisual;
    ...
    this.iconElement.dataset.inventoryVisual = selectedItemId;
    this.visualCanvases.push(this.iconElement);
  ...
  renderAnimation(elapsedMs) {
    if (!this.isOpen) return;
    for (const canvas of this.visualCanvases) {
      const itemId = canvas.dataset.inventoryVisual;
      if (!itemId) continue;
      this.renderItemVisual(canvas.getContext("2d"), itemId, elapsedMs);
    }
  }
}
