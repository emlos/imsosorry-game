...
export const MUSIC_RESTART_POLICIES = new Set(["always", "if-different", "never"]);
export const MUSIC_TRANSITION_POLICIES = new Set(["inherit", "replace", "crossfade", "silence"]);
export const MUSIC_EVENT_FREQUENCIES = new Set(["once-per-visit", "first-entry", "once-per-save"]);
...
function validateMusicConfig(config, label, { allowMissingTrack = false } = {}) {
    ...
    if (config.restart !== undefined && !MUSIC_RESTART_POLICIES.has(config.restart)) {
...
function validateMusicDefinition(music, label) {
    ...
    if (fallbackIndexes.length !== 1 || fallbackIndexes[0] !== music.length - 1) {
        throw new Error(`${label} must end with exactly one unconditional fallback entry.`);
    }
}
...
function validateMusicEvent(event, map, index) {
    ...
    requireExactKeys(
        event,
        new Set(["id", "frequency", "entryId", "probability", "condition", "effects"]),
        label,
    );
    requireString(event.id, `${label}.id`);

    if (event.frequency !== undefined && !MUSIC_EVENT_FREQUENCIES.has(event.frequency)) {
        throw new Error(
    ...
    if (event.entryId !== undefined) {
        ...
        if (!Object.hasOwn(map.entries, event.entryId)) {
            throw new Error(`${label}.entryId references missing entry "${event.entryId}".`);
        }
    }
...
export function validateMapMusicDefinition(map) {
    ...
    if (map.musicTransitionMs !== undefined) {
        requireNonNegativeNumber(map.musicTransitionMs, `Map "${map.id}".musicTransitionMs`);
    }
    ...
    map.musicEvents.forEach((event, index) => {
        ...
        if (ids.has(event.id)) {
            throw new Error(`Map "${map.id}" contains duplicate music event ID "${event.id}".`);
        }
        ids.add(event.id);
    });
}
...
export function resolveMapMusic(music, evaluate) {
    ...
    if (!Array.isArray(music)) return { kind: "play", options: withoutCondition(music) };

    const fallback = withoutCondition(music.at(-1));
    const conditional = music.slice(0, -1).find((config) => evaluate(config.condition));

    return {
        kind: "play",
        options: conditional ? { ...fallback, ...withoutCondition(conditional) } : fallback,
    };
}
