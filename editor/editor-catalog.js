...
export const INTERACTION_TEMPLATES = [
  ...
  },
  {
    ...
    label: "Teleport",
    description:
      "Direct action teleport to the first available entry, preferring another map.",
  ...
  },
  {
    ...
    label: "Item pickup",
    description:
      "Adds one item, shows pickup text, then deactivates this entity.",
  ...
  },
  {
    ...
    description:
      "Toggles an entity-specific flag and reports its generated flag ID.",
  ...
  },
  {
    ...
    label: "Inspect once",
    description:
      "Shows text once per save and sets an entity-specific flag after closing.",
  ...
  },
  {
    ...
    description:
      "Two mutually exclusive text effects selected by an entity-specific flag.",
...
export function getInteractionTemplate(templateId) {
  return (
    INTERACTION_TEMPLATES.find((template) => template.id === templateId) ?? null
  );
}
...
export function createInteractionFromTemplate(templateId, context = {}) {
  ...
  if (!template)
    throw new Error(`Unknown interaction template "${templateId}".`);
...
export const TILE_EDITOR_META = {
  ...
  [TILE_IDS.GLITTERING_CRYSTAL]: {
    label: "Glittering crystal (decorative)",
    category: "Decor",
  },
  [TILE_IDS.PLACEHOLDER_OBSTACLE]: {
    label: "Placeholder obstacle",
    category: "Debug",
  },
