...
export class EditorRenderer {
  ...
  async loadDefinitions(map) {
    const paths = new Set();
    for (const tile of Object.values(mergeTileDefinitions(map)))
      paths.add(tile.path);
  ...
  render(map, options = {}) {
    ...
    const visibleLayers =
      options.visibleLayers ?? new Set(Object.keys(map.layers));
    ...
    for (const entity of map.entities) {
      ...
      drawables.push({
        ...
        entity,
        depthY:
          (Math.max(...occupiedCells.map((cell) => cell.row)) + 1) * TILE_SIZE,
        sequence: sequence++,
      });
    }
    ...
    if (options.showCollision)
      this.renderCollisionOverlay(map, tiles, visibleLayers);
    ...
    if (options.showExits !== false)
      this.renderExits(map, options.selectedExitIndex);
    if (options.selectedEntityId)
      this.renderEntitySelection(map, options.selectedEntityId);
    if (options.rectanglePreview)
  ...
  renderTriggers(map, selectedTriggerId) {
    ...
    for (const [index, trigger] of (map.triggers ?? []).entries()) {
      ...
      ctx.fillStyle = selected
        ...
        : "rgba(130, 80, 220, 0.20)";
      ctx.strokeStyle = selected
        ? "rgba(225, 185, 255, 0.98)"
        : "rgba(180, 125, 245, 0.82)";
