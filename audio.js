...
function validateMusicRegistry(registry, label, { allowLoop }) {
  ...
  for (const [audioId, definition] of Object.entries(registry)) {
    ...
    if (
      ...
      throw new Error(
        `${entryLabel} must set loop: true when defining loop points.`,
      );
    }
  }
}
...
export class AudioSystem {
  constructor(soundDefinitions, musicDefinitions, musicEffectDefinitions = {}) {
    ...
    validateMusicRegistry(musicDefinitions, "Music registry", {
      allowLoop: true,
    });
    ...
    this.musicEffectDefinitions = new Map(
      Object.entries(musicEffectDefinitions),
    );
    ...
    window.addEventListener("keydown", this.resumeSoundContext, {
      capture: true,
    });
    window.addEventListener("pointerdown", this.resumeSoundContext, {
      capture: true,
    });
  }
  ...
  async unlock() {
    ...
    if (!this.unlocked) {
      await Promise.all(
        [...this.musicChannels, this.musicEffectChannel].map(
          async (channel) => {
            const { audio } = channel;
            audio.muted = true;
    ...
    const loadedBuffers = await Promise.all(
      [...this.soundDefinitions].map(async ([soundId, path]) => {
        ...
        try {
          ...
          throw new Error(
            `Could not decode sound "${soundId}" from "${path}".`,
            {
              cause: error,
            },
          );
  ...
  async loadMediaMetadata() {
    const entries = [
      ...[...this.musicDefinitions].map(([id, definition]) => [
        "music",
        id,
        definition,
      ]),
    ...
    const durations = await Promise.all(
      entries.map(async ([kind, id, definition]) => {
        const duration = await waitForMediaMetadata(
          definition.path,
          `${kind} "${id}"`,
        );
        if (
  ...
  flushPendingSounds() {
    if (
      this.soundContext.state !== "running" ||
      this.pendingSoundIds.length === 0
    ) {
      return;
    }
  ...
  normalizeMusicOptions(options) {
    ...
    if (normalized.continuityId !== null) {
      requireString(
        normalized.continuityId,
        "Music playback options.continuityId",
      );
    }
    for (const key of ["fadeInMs", "fadeOutMs", "crossfadeMs"]) {
      requireNonNegativeNumber(
        normalized[key],
        `Music playback options.${key}`,
      );
    }
    ...
    requireRange(
      ...
      0.25,
      4,
      "Music playback options.playbackRate",
    );
    if (normalized.position !== null) {
      requireNonNegativeNumber(
        normalized.position,
        "Music playback options.position",
      );
    }
  ...
  updatePreservedChannel(channel, options) {
    ...
    this.fadeChannelTo(
      channel,
      this.getActiveChannelGain(channel),
      options.fadeInMs,
    );
  }
  ...
  async playMusic(rawOptions) {
    ...
    try {
      ...
      this.setChannelGain(
        next,
        usesFadeIn ? 0 : this.getActiveChannelGain(next),
      );
    ...
    void this.fadeChannelTo(
      next,
      this.getActiveChannelGain(next),
      fadeDuration,
    );
  }
  ...
  async prepareMusicChannel(channel, options, definition, position) {
    ...
    await waitForChannelMetadata(
      channel,
      definition.path,
      `music "${options.trackId}"`,
    );

    channel.audio.loop =
      definition.loop !== false && definition.loopEnd === undefined;
    channel.audio.playbackRate = options.playbackRate;
    const duration = Number.isFinite(channel.audio.duration)
      ? channel.audio.duration
      : null;
    const maximum = definition.loopEnd ?? duration;
    const safePosition =
      maximum === null
        ? position
        : Math.min(position, Math.max(0, maximum - 0.01));
  ...
  rememberChannelPosition(channel) {
    if (!channel?.trackId || !Number.isFinite(channel.audio.currentTime))
      return;
    this.rememberedPositions.set(
      channel.trackId,
      Math.max(0, channel.audio.currentTime),
    );
  }
  ...
  captureMusicState() {
    ...
    return {
      ...
      position: Number.isFinite(channel.audio.currentTime)
        ? channel.audio.currentTime
        : 0,
  ...
  validateSaveState(value, label = "Save data.music") {
    ...
    requireExactKeys(
      value,
      new Set(["current", "rememberedPositions", "stack"]),
      label,
    );
    ...
    const validateSnapshot = (snapshot, snapshotLabel) => {
      ...
      if (!this.hasMusic(snapshot.trackId)) {
        throw new Error(
          `${snapshotLabel} references missing music "${snapshot.trackId}".`,
        );
      }
    ...
    for (const [trackId, position] of Object.entries(
      value.rememberedPositions,
    )) {
      ...
      requireNonNegativeNumber(
        position,
        `${label}.rememberedPositions.${trackId}`,
      );
  ...
  async restoreSaveState(state) {
    ...
    this.rememberedPositions = new Map(
      Object.entries(prepared.rememberedPositions),
    );
