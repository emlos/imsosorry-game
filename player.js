...
export class Player {
  ...
  beginStep(dc, dr) {
    if (this.isMoving) {
      throw new Error(
        "Cannot start a player step while another step is active.",
      );
    }
    if (
      !Number.isInteger(dc) ||
      ...
      throw new Error(
        "Player movement steps must use a non-zero unit input vector.",
      );
    }
    ...
    this.moveElapsedMs = 0;
    this.moveDurationMs =
      (1000 * Math.hypot(dc, dr)) /
      (this.state.movementSpeed * MOVEMENT_SUBDIVISIONS);
    this.isMoving = true;
  }
  ...
  update(deltaMs) {
    ...
    return {
      ...
      tileChanged:
        currentTile.col !== previousTile.col ||
        currentTile.row !== previousTile.row,
      previousTile,
      currentTile,
    };
  }
