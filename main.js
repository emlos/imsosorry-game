...
const EDITOR_PLAYTEST_STORAGE_KEY = "yume-map-editor-playtest-maps-v7";
const EDITOR_PLAYTEST_RESULT_KEY = "yume-map-editor-playtest-result-v7";
...
function isEditorPlaytest() {
  return (
    new URLSearchParams(window.location.search).get("editorPlaytest") === "1"
  );
}
...
function getAuthoredMaps() {
  ...
  if (!Array.isArray(parsed) || parsed.length === 0) {
    throw new Error(
      "The editor playtest document is not a nonempty map array.",
    );
  }

  document.querySelector("#status").textContent =
    "Editor playtest data loaded.";
  return parsed;
}
